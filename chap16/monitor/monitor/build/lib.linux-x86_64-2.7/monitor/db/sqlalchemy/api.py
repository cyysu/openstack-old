...
def osd_get_all(context, session=None):
    query =  model_query(context, models.OsdState)
    return query.all()
...
# opreration on Monitor user capacity management
@require_admin_context
def monitor_capacity_manage_get(context, monitor_capacity_manage_id, session=None):
    result = model_query(context, models.VsmCapacityManage, session=session).\
                     filter_by(id=monitor_capacity_manage_id).\
                     first()

    if not result:
        raise exception.VsmCapacityManageNotFound(monitor_capacity_manage=
                                                  monitor_capacity_manage_id)

    return result


@require_admin_context
def monitor_capacity_manage_get_all(context, session=None):
    return model_query(context, models.VsmCapacityManage, session=session).\
                    all()

@require_admin_context
def monitor_capacity_manage_create(context, values, session=None):
    if not session:
        session = get_session()

    with session.begin(subtransactions=True):
        monitor_capacity_manage_ref = models.VsmCapacityManage()
        session.add(monitor_capacity_manage_ref)
        monitor_capacity_manage_ref.update(values)

    return monitor_capacity_manage_ref


@require_admin_context
def monitor_capacity_manage_update(context, monitor_capacity_manage_id, values):    
    session = get_session()

    with session.begin(subtransactions=True):
        values['updated_at'] = timeutils.utcnow()
        convert_datetimes(values, 'created_at', 'deleted_at', 'updated_at')
        monitor_capacity_manage_ref = monitor_capacity_manage_get(
                                   context, monitor_capacity_manage_id,
                                   session=session)

        for (key, value) in values.iteritems():
            monitor_capacity_manage_ref[key] = value
        monitor_capacity_manage_ref.save(session=session)


@require_admin_context
def monitor_capacity_manage_delete(context, monitor_capacity_manage_id):    
    session = get_session()

    with session.begin(subtransactions=True):
        monitor_capacity_manage_ref = monitor_capacity_manage_get(context, monitor_capacity_manage_id, session=session)
        monitor_capacity_manage_ref['deleted_at'] = timeutils.utcnow()
        monitor_capacity_manage_ref['updated_at'] = monitor_capacity_manage_ref['deleted_at']
        monitor_capacity_manage_ref['deleted'] = True
        convert_datetimes(monitor_capacity_manage_ref, 'created_at', 'deleted_at', 'updated_at')
