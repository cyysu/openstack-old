...
def upgrade(migrate_engine):
    ...
    monitor_capacity_manages = Table(
        'monitor_capacity_manages', meta,
        Column('id', Integer, primary_key=True, nullable=False),
    ...
    try:
        ...
        LOG.info(repr(recipes))
        LOG.exception('Exception while creating table')
        meta.drop_all(tables=[recipes])
        raise

    try:
        monitor_capacity_manages.create()
    except Exception:
        LOG.info(repr(monitor_capacity_manages))
        LOG.exception('Exception while creating table')
        meta.drop_all(tables=[monitor_capacity_manages])
        raise
...
def downgrade(migrate_engine):
    ...
    monitor_capacity_manages = Table('monitor_capacity_manages',
                                meta,
                                autoload=True)
    monitor_capacity_manages.drop()
