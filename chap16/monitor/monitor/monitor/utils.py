...
def find_config(config_path):
    ...
    :raises: `monitor.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(FLAGS.state_path, "etc", "monitor", config_path),
        os.path.join(FLAGS.state_path, "etc", config_path),
        os.path.join(FLAGS.state_path, config_path),
        "/etc/monitor/%s" % config_path,
    ]
...
def execute(*cmd, **kwargs):
    ...
    corresponding filter to etc/monitor/rootwrap.d !
...
def monitordir():
    import monitor
    return os.path.abspath(monitor.__file__).split('monitor/__init__.py')[0]
...
def last_completed_audit_period(unit=None):
    ...
    if not unit:
        unit = FLAGS.servicemanage_usage_audit_period
...
def monkey_patch():
    ...
    Example: 'monitor.api.ec2.cloud:' \
     monitor.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See monitor.openstack.common.notifier.api.notify_decorator)
...
def generate_mac_address():
    ...
    #             properly: 0xfa.
    #             Discussion: https://bugs.launchpad.net/monitor/+bug/921838
    mac = [0xfa, 0x16, 0x3e,
